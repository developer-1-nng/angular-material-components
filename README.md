# Angular Material Extra Components (DatetimePicker, TimePicker, ColorPicker, FileInput ...) for @angular/material 16.x


## Description

Angular Material Library provide extra components for every project (Datetime picker, Time picker, Color picker...).

Please do not hesitate to contact us if you have any question or suggestion.

Feel free to open the [issues](https://github.com/developer-1-nng/angular-material-components/issues) or provide a PR.

If you like this project, support angular-material-components by starring⭐.


## DEMO Angular Material Components

@see [LIVE DEMO AND DOCUMENTATION](https://github.com/developer-1-nng/angular-material-components/)

Choose the version corresponding to your Angular version:

 Angular     | @nng-components/angular-material-datetime-picker
 ----------- | -------------------
 16          | 16.x+


### Datetime Picker


[TUTORIAL HERE](https://github.com/developer-1-nng/angular-material-components/)

```
npm install --save  @nng-components/angular-material-datetime-picker
```

![Alt Text](demo_datetime_picker.png)


### Color Picker


[TUTORIAL HERE](https://github.com/developer-1-nng/angular-material-components/)

```
npm install --save  @nng-components/angular-material-color-picker
```

![Alt Text](demo_color_picker.png)

### File Input


[TUTORIAL HERE](https://github.com/developer-1-nng/angular-material-components/)

```
npm install --save  @nng-components/angular-material-file-input
```

![Alt Text](demo_file_input.png)


### Theming
- @see @angular/material [Using a pre-built theme](https://material.angular.io/guide/theming#using-a-pre-built-theme)
- Add the Material Design icon font to your index.html
```
<link href="https://fonts.googleapis.com/icon?family=Material+Icons&display=block" rel="stylesheet">
```

## License
MIT
